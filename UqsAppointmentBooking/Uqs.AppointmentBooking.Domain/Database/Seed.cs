...
public class Seed
{
    public static async Task Initialize(IServiceProvider serviceProvider)
    {
        ...
        // Employees
        var tom = await context.AddAsync(new Employee { Name = "Tomas Fringe" });
        var jane = await context.AddAsync(new Employee { Name = "Jane Higgins" });
        var will = await context.AddAsync(new Employee { Name = "Will Turner" });
        var jess = await context.AddAsync(new Employee { Name = "Jessica Howard" });
        var ed = await context.AddAsync(new Employee { Name = "Edward Gardens" });
        var oli = await context.AddAsync(new Employee { Name = "Oliver Owl" });
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            // Tom works all shifts except Saturday
            if (date.DayOfWeek != DayOfWeek.Saturday)
            {
                await context.AddAsync(new Shift
                    { Employee = tom.Entity, Starting = SetTime(date, "09:00"), Ending = SetTime(date, "12:00") });
                await context.AddAsync(new Shift
                    { Employee = tom.Entity, Starting = SetTime(date, "13:00"), Ending = SetTime(date, "18:00") });
            }

            // Jane works at peak times only   
            await context.AddAsync(new Shift
                { Employee = jane.Entity, Starting = SetTime(date, "11:00"), Ending = SetTime(date, "14:00") });

            // Will works Friday and Saturday only
            if (date.DayOfWeek == DayOfWeek.Friday || date.DayOfWeek == DayOfWeek.Saturday)
            {
                await context.AddAsync(new Shift
                    { Employee = will.Entity, Starting = SetTime(date, "09:00"), Ending = SetTime(date, "12:00") });
                await context.AddAsync(new Shift
                    { Employee = will.Entity, Starting = SetTime(date, "13:00"), Ending = SetTime(date, "18:00") });
            }

            // Jess works all week but takes a longer lunch break
            await context.AddAsync(new Shift
                { Employee = jess.Entity, Starting = SetTime(date, "09:00"), Ending = SetTime(date, "11:30") });
            await context.AddAsync(new Shift
                { Employee = jess.Entity, Starting = SetTime(date, "14:30"), Ending = SetTime(date, "18:00") });

            // Ed works morning shifts
            await context.AddAsync(new Shift
                { Employee = ed.Entity, Starting = SetTime(date, "09:00"), Ending = SetTime(date, "12:00") });

            // Oli works afternoon shifts
            await context.AddAsync(new Shift
                { Employee = oli.Entity, Starting = SetTime(date, "13:00"), Ending = SetTime(date, "18:00") });
        }
