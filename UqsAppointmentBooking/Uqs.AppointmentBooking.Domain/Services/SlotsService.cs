...
public class SlotsService
{
    ...
    public async Task<Slots> GetAvailableSlotsForEmployee(int serviceId, int employeeId)
    {
        ...
        if (service is null) throw new ArgumentException("Record not found", nameof(serviceId));

        var isEmpFound = await _context.Employees!.AnyAsync(x => x.Id == employeeId);
        if (!isEmpFound) throw new ArgumentException("Record not found", nameof(employeeId));
        ...
        var appointments = _context.Appointments!.Where(
            x => x.EmployeeId == employeeId
                 && x.Ending < appointmentsMaxDay
    ...
    private DateTime GetEndOfOpenAppointments() => _now.Date.AddDays(_settings.OpenAppointmentInDays);
